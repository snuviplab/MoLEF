...
class Didemo(Dataset):
    def __init__(self, model_name, feature_path, data_path, word2vec, max_num_frames, max_num_words, max_num_nodes, is_training, is_adj, vocab_size, with_max_IoU=-float('inf')):
        ...
        self.anchors = generate_anchors(dataset='Didemo')
        ...
        # cal words id 
        self.vocab = load_pkl(file_path="path/to/data/didemo/did_glove.pkl")
    ...
    def __getitem__(self, ind):
        ...
        # video feature
        vid_feats = load_feature(os.path.join(self.feature_path, 'fc7_subsample5_fps25_'+'%s.npy' % vid), dataset='Didemo')
        ...
        if self.model_name == 'tga':
            # To avoid error ('Down-sampling factors must be >= 1') min value of seg_size = 1
            seg_size = max(1, int(fps * 7.52) // 2) # average moment length of DiDeMo 7.52
