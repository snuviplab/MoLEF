...
class Tacos(Dataset):
    def __init__(self, model_name, feature_path, data_path, word2vec, max_num_frames, max_num_words, max_num_nodes, is_training, is_adj, vocab_size, with_max_IoU=-float('inf')):
        ...
        # cal words id 
        self.vocab = load_pkl(file_path="path/to/data/tacos/tac_glove.pkl")
    ...
    def __getitem__(self, ind):
        ...
        # video feature
        vid_feats = load_feature(os.path.join(self.feature_path, '%s.npy' % vid[:-4]), dataset='Tacos')
        ...
        if self.model_name == 'tga':
            seg_size = int(fps * 6.10) // 2 # average moment length of TACoS 6.10
        ...
        if self.model_name in ['cpl', 'cnm']:
            ...
            tmp = np.exp(weights[:valid_words_length])
            sum_tmp = np.sum(tmp)
