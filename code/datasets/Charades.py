...
class Charades(Dataset):
    def __init__(self, model_name, feature_path, data_path, word2vec, max_num_frames, max_num_words, max_num_nodes, is_training, is_adj, vocab_size, with_max_IoU=-float('inf')):
        ...
        self.vocab = load_pkl(file_path="path/to/data/charades/char_glove.pkl")
    ...
    def __getitem__(self, ind):
        ...
        # video feature
        vid_feats = load_feature(os.path.join(self.feature_path, '%s.npy' % vid), dataset='Charades')
        ...
        if self.model_name == 'tga':
            seg_size = int(fps * 8.23) // 2 # average moment length of Charades-STA 8.23
        ...
        if self.is_adj != False:  #  ianet, csmgan use this setting
            ...
            adj_mat[adj_mat < self.is_adj * max_IoU] = 0.  # ianet : self.is_adj = 0.45, csmgan : self.is_adj = 0.8
