...
import h5py, jsonlines # query embedding
...
class Tvr(Dataset):
    def __init__(self, model_name, feature_path, data_path, word2vec, max_num_frames, max_num_words, max_num_nodes, is_training, is_adj, vocab_size, with_max_IoU=-float('inf')):
        ...
        self.anchors = generate_anchors(dataset='Tvr')
        ...
        # path to tvr annotation jsonl file
        # tvr_release.json = tvr_train_release.jsonl + tvr_val_release.jsonl from TVR official github
        self.desc_id_path = 'path/to/tvr/annot/tvr_release.jsonl'
        self.desc_id = {}
        with jsonlines.open(self.desc_id_path) as f:    
            for line in f:
                self.desc_id[line['vid_name']] = line['desc_id']
        
        # path to directory of npy feature for query_only 
        self.query_embed_path = 'path/to/data/tvr/query_only'
        self.sub_embed_path = None
        
        # path to directory of npy feature for query_w_sub, sub_query
        if self.model_name == 'reloclnet':
            self.query_embed_path = 'path/to/data/tvr/query_w_sub'
            self.sub_embed_path = 'path/to/data/tvr/sub_query'
        
        # path to glove pkl file for tvr
        # cal words id 
        self.vocab = load_pkl(file_path="path/to/data/tvr/tvr_glove.pkl")
    ...
    def __getitem__(self, ind):
        ...
        # video feature
        vid_feats = load_feature(os.path.join(self.feature_path, '%s.npy' % vid), dataset='Tvr')
        ...
        # word embedding 
        # # words_vec = np.asarray(self.embed_data[str(self.desc_id[vid])])
        desc_id = self.desc_id[vid]
        words_vec = load_feature(os.path.join(self.query_embed_path, "%s.npy" % desc_id), dataset='Tvr')
        ...
        # reloclnet
        sub_feats = np.zeros([self.max_num_frames, words_vec.shape[1]], dtype=np.uint8)
        sub_mask = np.zeros([self.max_num_frames], dtype=np.uint8)
        ...
        if self.model_name == 'reloclnet':
            ...
            sub_feats = load_feature(os.path.join(self.sub_embed_path, '%s.npy' % vid), dataset='Tvr')
            sub_feats = uniform_feature_sampling(sub_feats, self.max_num_frames)
            
            if sub_feats.shape[0] < self.max_num_frames :
                sub_mask = np.zeros([self.max_num_frames], np.uint8)
                sub_mask[:sub_feats.shape[0]] = 1
                sub_feats = np.pad(sub_feats, 
                                ((0,self.max_num_frames-sub_feats.shape[0]), (0,0)))
            else :    
                sub_mask = np.ones([self.max_num_frames], np.uint8)
            
            fr_label = get_st_ed_label(ts=timestamps, max_idx=ctx_l-1, clip_length=1.5)
    
        # tga - video features 
        seg_size = 0
        seg_break = [0, 0, 0]
        if self.model_name == 'tga':
            seg_size = int(fps * 9.10) // 2 # average moment length of TVR 9.10
        ...
        words_feat = np.concatenate([np.zeros((1,768)), words_vec], axis = 0).astype(np.float32)      
        words_feat[-1] = np.zeros((1,768))
