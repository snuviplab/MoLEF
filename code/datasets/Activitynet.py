...
class Activitynet(Dataset):
    def __init__(self, model_name, feature_path, data_path, word2vec, max_num_frames, max_num_words, max_num_nodes, is_training, is_adj, vocab_size, with_max_IoU=-float('inf')):
        ...
        self.vocab = load_pkl(file_path="path/to/data/activitynet/act_glove.pkl")
    ...
    def __getitem__(self, ind):
        ...
        # video feature
        vid_feats = load_feature(os.path.join(self.feature_path, '%s.npy' % vid), dataset='Activitynet')
        ...
        if self.model_name == 'tga':
            seg_size = int(fps * 37.14) // 2 # average moment length of ActivityNet 37.14
