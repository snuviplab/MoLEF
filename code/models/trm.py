# from https://github.com/minghangz/TRM

import torch
from torch import nn
from torch.functional import F
from torch.nn.utils.rnn import pad_sequence
from modules.trm.modeling.featpool import build_featpool  # downsample 1d temporal features to desired length
from modules.trm.modeling.feat2d import build_feat2d  # use MaxPool1d/Conv1d to generate 2d proposal-level feature map from 1d temporal features
from modules.trm.modeling.loss import build_contrastive_loss
from modules.trm.modeling.loss import build_bce_loss
from modules.trm.modeling.text_encoder import build_text_encoder # modules.trm.modeling.text_encoder/text_encoder_glove/text_encoder_tvr
from modules.trm.modeling.proposal_conv import build_proposal_conv
import random
...
class Model(nn.Module):
    def __init__(self, cfg):
        ...
        self.w = self.cfg.MODEL.TRM.RESIDUAL

    def forward(self, feats, queries, wordlens, all_iou2d, moments, sentences, phrases, cur_epoch=1):
        """
        Arguments:
        ...
        # backbone
        ious2d = all_iou2d
        assert len(ious2d) == feats.size(0)
        for idx, (iou, sent) in enumerate(zip(ious2d, queries)):
            assert iou.size(0) == sent.size(0)
            # assert iou.size(0) == num_sentence[idx]
        ...
        if self.text_encoder.use_phrase:
            sent_feat, sent_feat_iou, phrase_feat, phrase_feat_iou, phrase_weight, phrase_mask = self.text_encoder.encode_sentences(sentences, phrases)
        else:
            sent_feat, sent_feat_iou = self.text_encoder(queries, wordlens)
        # inference
        contrastive_scores = []
        iou_scores = []
        phrase_iou_scores = []
        ...
        for i, sf_iou in enumerate(sent_feat_iou):  # sent_feat_iou: [num_sent x C] (len=B)
            # iou part
            if self.cfg.MODEL.TRM.RESIDUAL and self.text_encoder.use_phrase:
        ...
        # phrase contrastive
        if self.text_encoder.use_phrase and self.cfg.MODEL.TRM.LOSS.CONTRASTIVE:
            contrast_neg_vid = []
            contrast_neg_phr = []
            contrast_neg_mask = []
        ...
        if self.text_encoder.use_phrase and self.use_score_map_loss:
            ...
            if self.cfg.MODEL.TRM.LOSS.USE_FOCAL_LOSS:
            ...
            if self.cfg.MODEL.TRM.LOSS.CONTRASTIVE:
        ...
        # import pdb
        # pdb.set_trace()
        if self.text_encoder.use_phrase and not self.cfg.MODEL.TRM.RESIDUAL:
            loss_iou_phrase = self.iou_score_loss(torch.cat(phrase_iou_scores, dim=0), torch.cat(ious2d, dim=0), cur_epoch)
            if not self.cfg.MODEL.TRM.LOSS.PHRASE_ONLY:
        ...
        loss_vid, loss_sent = self.contrastive_loss(map2d, sent_feat, ious2d, None, None, moments)
        
        contr_weight = self.cfg.MODEL.TRM.LOSS.CONTRASTIVE_WEIGHT
        consis_weight = self.cfg.MODEL.TRM.LOSS.CONSIS_WEIGHT
        exc_weight = self.cfg.MODEL.TRM.LOSS.EXC_WEIGHT
        ...
        loss = 0
        if cur_epoch <= self.cfg.SOLVER.ONLY_IOU:
        ...
        loss_dict = {
            'loss_vid': loss_vid, 
            'loss_sent': loss_sent, 
            'loss_iou_stnc': loss_iou_stnc, 
            'loss_iou_phrase': loss_iou_phrase, 
            'scoremap_loss_pos': scoremap_loss_pos, 
            'scoremap_loss_neg': scoremap_loss_neg, 
            'scoremap_loss_exc': scoremap_loss_exc
        }
        ...
        if self.training:
            ...
            if self.text_encoder.use_phrase and not self.cfg.MODEL.TRM.RESIDUAL:
                ...
                if self.cfg.MODEL.TRM.LOSS.PHRASE_ONLY:
                    return map2d_iou, sent_feat_iou, contrastive_scores, phrase_iou_scores, loss_dict, loss  # first two maps for visualization
                else:
                    ...
                    return map2d_iou, sent_feat_iou, contrastive_scores, iou_scores, loss_dict, loss # first two maps for visualization
            else:
                return map2d_iou, sent_feat_iou, contrastive_scores, iou_scores, loss_dict, loss
