...
from modules.tmlga.modeling.dynamic_filters.build import DynamicFilter
from modules.tmlga.utils import loss as L
from modules.tmlga.utils.rnns import feed_forward_rnn
import modules.tmlga.utils.pooling as POOLING
...
class Model(nn.Module):
    ...
    def attention(self, videoFeat, filter, lengths):
        ...
        pred_local = torch.bmm(videoFeat, filter.unsqueeze(2)).squeeze(dim=2) 
        return pred_local
    ...
    def forward(self, videoFeat, videoFeat_lengths, tokens, tokens_lengths, start, end, localiz):
        ...
        pred_start = self.starting(output.view(-1, output.size(2))).view(-1,output.size(1),1).squeeze(dim=2)
        ...
        pred_end = self.ending(output.view(-1, output.size(2))).view(-1,output.size(1),1).squeeze(dim=2)
