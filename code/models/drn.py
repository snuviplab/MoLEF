...
from modules.drn.language_module import QueryEncoder
from modules.drn.graph_module import LanguageGuidedGraphNetwork
from modules.drn.backbone import Backbone
from modules.drn.basic_blocks import conv_with_kaiming_uniform
from modules.drn.fcos import build_fcos
from modules.drn.FPN import FPN


class Model(nn.Module):
    # dataset_configs
    def __init__(self, vocab_size, args, hidden_dim=512, embed_dim=300, bidirection=True,
                 graph_node_features=1024):
        super(Model, self).__init__()
        self.args = args
        self.first_output_dim = args.first_output_dim
        self.fpn_feature_dim = args.fpn_feature_dim
        self.query_encoder = QueryEncoder(vocab_size, hidden_dim, args.word_dim, args.lstm_layers, bidirection)

        if args.dataset == 'Activitynet' :
            self.feature_dim = 500
        elif args.dataset == 'Charades':
            self.feature_dim = 1024
            # self.feature_dim = 4096 # for vgg
        elif args.dataset in ['Tacos','Didemo', 'Msrvtt']:
        ...
        self.fcos = build_fcos(vars(args), self.fpn_feature_dim)
    ...
    def forward(self, query_tokens, query_length, props_features, props_start_end, gt_start_end):
