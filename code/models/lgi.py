# from https://github.com/JonghwanMun/LGI4temporalgrounding

import os
from collections import OrderedDict

from modules.lgi import building_blocks as bb
from modules.lgi.abstract_network import AbstractNetwork
from modules.lgi.utils import io_utils, net_utils, vis_utils

class Model(AbstractNetwork):
    def __init__(self, config, logger=None, verbose=True):
        """ Initialize baseline network for Temporal Language Grounding
        """
        super(Model, self).__init__(config=config, logger=logger)
    ...
    def _build_network(self):
        ...
        self.nse = getattr(mconfig, "num_semantic_entity", -1) 
        if self.nse > 1:
        ...
        # build criterion
        self.use_tag_loss = getattr(mconfig, "use_temporal_attention_guidance_loss", True) 
        self.use_dqa_loss = getattr(mconfig, "use_distinct_query_attention_loss", True)
    ...
    def reset_status(self, init_reset=False):
        ...
        """
        super(Model, self).reset_status(init_reset=init_reset)
    ...
    def save_results(self, prefix, mode="Train"):
        # save predictions
        # save_dir = os.path.join(self.config.misc.result_dir, "predictions", mode) # self.config["misc"]["result_dir"]
    ...
    def bring_dataset_info(self, dset):
        super(Model, self).bring_dataset_info(dset)
    ...
    def model_specific_config_update(self, config):
        ...
        self.lgi_fusion_method = getattr(mconfig, "lgi_fusion_method", "concat")
        ...
        l_type = getattr(mconfig, "lgi_local_type", "res_block")
        if l_type == "res_block":
        ...
        g_type = getattr(mconfig, "lgi_global_type", "nl")
        ...
        if getattr(mconfig, "lgi_global_satt_att_use_embedding", False):
