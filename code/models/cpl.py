# from https://github.com/minghangz/cpl
...
from modules.cpl.transformer import DualTransformer
from modules.cpl.loss import rec_loss, ivc_loss, cal_nll_loss
import math
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.sigma = config.cpl.config.sigma
        self.use_negative = config.cpl.config.use_negative
        self.num_props = config.cpl.config.num_props
        self.max_epoch = config.train.max_num_epochs
        self.gamma = config.cpl.config.gamma

        self.frame_fc = nn.Linear(config.cpl.config.frames_input_size, config.cpl.config.hidden_size)
        self.word_fc = nn.Linear(config.cpl.config.words_input_size, config.cpl.config.hidden_size)
        self.mask_vec = nn.Parameter(torch.zeros(config.cpl.config.words_input_size).float(), requires_grad=True)
        self.start_vec = nn.Parameter(torch.zeros(config.cpl.config.words_input_size).float(), requires_grad=True)
        self.pred_vec = nn.Parameter(torch.zeros(config.cpl.config.frames_input_size).float(), requires_grad=True)

        self.trans = DualTransformer(**config.cpl.config.DualTransformer)
        self.fc_comp = nn.Linear(config.cpl.config.hidden_size, self.vocab_size)
        self.fc_gauss = nn.Linear(config.cpl.config.hidden_size, self.num_props*2)

        self.word_pos_encoder = SinusoidalPositionalEmbedding(config.cpl.config.hidden_size, 0, 20)
    ...
    def forward(self, frames_feat, frames_len, words_id, words_feat, words_len, weights, **kwargs):
        ...
        loss, loss_dict = rec_loss(**output, num_props=self.num_props, **self.config_loss)
        rnk_loss, rnk_loss_dict = ivc_loss(**output, num_props=self.num_props, **self.config_loss)
        loss_dict.update(rnk_loss_dict)
        loss = loss + rnk_loss
        words_mask = words_mask.unsqueeze(1) \
            .expand(bsz, self.num_props, -1).contiguous().view(bsz*self.num_props, -1)
        words_id = words_id.unsqueeze(1) \
            .expand(bsz, self.num_props, -1).contiguous().view(bsz*self.num_props, -1)   
        nll_loss, acc = cal_nll_loss(words_logit, words_id, words_mask)
        idx = nll_loss.view(bsz, self.num_props).argsort(dim=-1)

        width = output['width'].view(bsz, self.num_props).gather(index=idx, dim=-1)
        center = output['center'].view(bsz, self.num_props).gather(index=idx, dim=-1)
        selected_props = torch.stack([torch.clamp(center-width/2, min=0), 
                                        torch.clamp(center+width/2, max=1)], dim=-1)

        output['selected_props'] = selected_props
